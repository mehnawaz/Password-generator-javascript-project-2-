class password {
  constructor() {
	...
	funnyPassword(){
		let arr= ["password","incorrect","12345678","iforgot","secret","MyPreciousssPasssword","MyName","Chitu_Hacker","nothing","user","maiNahiBatunga","Sorry","1$perhour","admin","passwordIs1234", "RememberMe", "chutiya", "YaadHu", "WaahTaj", "LoliPop", "BoomBoomBoomer", "Pass", "Fail", "paGal", "Word", "LoJi", "YahiHay"]
	...
	strongPassword(){
		let arr= [this.upcl,this.lwcl,this.num]
    let pass= ""
    for(let i= 0;i<8;i++){
      let Ran1= Math.floor(Math.random()*arr.length)
      let Ran2= Math.floor(Math.random()*arr[Ran1].length)
